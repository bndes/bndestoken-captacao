...
contract('GovernanceInAction', async accounts => {
  ...
  it("should create the PreUpgrader contract with Governance - async", async () => {
    ...
    preUpgrader1 = await PreUpgrader1.new(bndesAddr,bndesAddr,governanceMembersId, bndesAddr);    
  ...
  /*
  it("if account does not exist, the state is true -- async", async () => {

    let b = await bndesRegistry.isAvailable(id2);

    assert.equal(b, true, "State is not Available " + b);
